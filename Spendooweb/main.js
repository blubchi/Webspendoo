...
function renderTransactions() {
    ...
    transactions.forEach(tx => {
        ...
        const categoryData = CATEGORIES[tx.type]?.find(c => c.name === tx.category) || { icon: ICONS.OTHERS };
        ...
        txElement.innerHTML = `
            <div class="tx-item-left">
                ...
                <div class="tx-item-details">
                    ...
                    <p class="tx-item-date">${formatDate(txDate, { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                </div>
            </div>
...
function openCategoryModal() {
    categoryModalTitle.textContent = Category ${currentTxType === 'income' ? 'Income' : 'Expense'};
...
function renderDatePicker() {
    ...
    datepickerMonthYear.textContent = formatDate(currentDatePickerDate, { month: 'long', year: 'numeric' });
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        if (day === currentTxDate.getDate() && month === currentTxDate.getMonth() && year === currentTxDate.getFullYear()) {
            btn.classList.add('selected');
        }
        
        if (day === today.getDate() && month === today.getMonth() && year === today.getFullYear() && !btn.classList.contains('selected')) {
...
function setupEventListeners() {
    ...
    timeHourUp.addEventListener('click', () => changeTime(timeHourDisplay, 1, 1, 12, true));
    timeHourDown.addEventListener('click', () => changeTime(timeHourDisplay, -1, 1, 12, true));
    timeMinuteUp.addEventListener('click', () => changeTime(timeMinuteDisplay, 1, 0, 59, true));
    timeMinuteDown.addEventListener('click', () => changeTime(timeMinuteDisplay, -1, 0, 59, true));
}

// === OTENTIKASI ===
// ... (Kode Otentikasi Anda tidak perlu diubah) ...
onAuthStateChanged(auth, (user) => {
  if (user) {
    ...
    // Asumsi halaman login adalah 'index.html'
